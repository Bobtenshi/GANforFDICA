...
class DuongDNN(object):
    ...
    def init_param(self):
        ...
        self.Rspace = np.einsum('pq,in->pqin', np.eye(self.M), np.ones([self.I, self.N]), dtype=np.complex128)

        LA.inv(np.random.randn(N, M) * np.exp(np.random.rand(N, M) * 2 * np.pi * 1j))
    ...
    def iterate(self):
        ...
        for it in range(1, self.n_iter+1):
            ...
            # todo duong_methodもDuongDNNクラスに組み込んで良いと思う
            chat, v, Rspace = duong_method(self.Xinput, self.N, n_iter=self.n_update_w, v=self.R, R=self.Rspace, evaluater=self.evaluater)
...
if __name__ == '__main__':
    ...
    p.add_argument('-w', '--n_update_w', metavar='N', type=int, default=20,
                   help='how many updates w per dnn predict (default: 20)')  # DNN1回ごとに何回duongのEMを通すか
